...
JsonRpcRequestProcessor.prototype._throwError = function (code, exception, id) {
    ...
    //TODO: tutaj if czy kod bledu jest z przedzialu zastrzezonych czy nie
    //jak z zastrzezonych to rzucamy standardowy message jak nie to bierzemy z expcetion

    if (Types.isObject(code)) {
        console.error('JsonRpcRequestProcessor/_throwError/ code: ' + code.code + " msg: " + code.message);
        jsonRpcResponse.setError(parseInt(code.code, 10), code.message || "", exception.data ? exception.data : null);
    } else {
        console.error('JsonRpcRequestProcessor/_throwError/ code: ' + code + " msg: " + exception.message);
        jsonRpcResponse.setError(parseInt(code, 10), exception.message || "", exception.data ? exception.data : null);
    }
