...
Request.prototype.setUrl = function (url) {
    ...
    //console.info("Core/Request/setUrl: url=%s, length=%d, referer=%s, buffer=", url, url.length, this.getHeader('referer'), new Buffer(url));
    // FIXME: parser (rfc) nie pozwala przeslac [ i ] trzeba urlencode
            ...
            if (query.hasOwnProperty(key)) {
                if (Array.isArray(query[key])) {
                    for (var i = 0, max = query[key].length; i < max; i++) {
                        ...
                        tmp += this._isEncoded(key) ? key : encodeURIComponent(key);
                        tmp += "[]";
                        tmp += "=" + (this._isEncoded(value) ? value : encodeURIComponent(value));
                        tmp += '&';
                    }
                } else {
                    tmp += this._isEncoded(key) ? key : encodeURIComponent(key);
                    ...
                    if (query[key]) {
                        tmp += "=" + (this._isEncoded(query[key]) ? query[key] : encodeURIComponent(query[key]));
                    } else {
                        tmp += "=";
                    }
...
Request.prototype.getQueryParam = function (name) {
    ...
    return (this._query.hasOwnProperty(name)) ? this._query[name] : null;
};
